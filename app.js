const jobs=[
  {id:'grace-stylist',title:'美容師スタイリスト',company:'Art Hair GRACE',area:'都城市',address:'宮崎県都城市栄町13-4',type:'正社員',category:'美容・理容',salary:'月給25万円保証／歩合45％',hours:'9:00〜18:30',holidays:'年間休日120日',status:'募集中',tone:'beauty',photo:'https://imgbp.hotp.jp/CSP/IMG_SRC_K/06/71/C015620671/C015620671.jpg?h=500&impolicy=HPB_policy_default&w=666',photoPosition:'center 52%',description:'都城駅近くの地域密着サロン。スタッフや店内の雰囲気、実際のスタイルはホットペッパービューティーで確認できます。',links:[{label:'ホットペッパーを見る',url:'https://beauty.hotpepper.jp/slnH000200941/'},{label:'場所を見る',url:'https://www.google.com/maps/search/?api=1&query=Art%20Hair%20Grace%20%E9%83%BD%E5%9F%8E%E5%B8%82%E6%A0%84%E7%94%BA13-4'}],contact:'mailto:weeds_skillup_0128@yahoo.co.jp?subject=Art Hair GRACE求人への応募相談'},
...
function renderJobVisual(job){
  const imageStyle=job.photo
    ? ` style="--job-photo:url('${job.photo}');--job-photo-position:${job.photoPosition||'center'}"`
    : '';
  return `<div class="job-visual ${job.tone}${job.photo?' has-photo':''}"${imageStyle}><small>${job.status}</small><strong>${job.company}</strong></div>`;
}
...
function renderJobs({animate=false}={}){
  ...
  if($('#jobList')){
    $('#jobList').innerHTML=list.map(job=>`<article class="job-card store-card" data-job-id="${job.id}">${renderJobVisual(job)}<div class="job-body"><p class="job-meta">${job.area}｜${job.type}</p><h3>${job.title}</h3><p class="job-company">${job.company}</p><p class="job-address">📍 ${job.address}</p><p class="job-description">${job.description}</p><strong class="job-salary">${job.salary}</strong><div class="job-conditions"><span>勤務時間　${job.hours}</span><span>休日　${job.holidays}</span></div><div class="shop-links">${renderLinks(job.links)}</div><a class="job-link apply-link" href="${job.contact}">応募・問い合わせ →</a></div></article>`).join('');
  }
...
(()=>{
  const style=document.createElement('style');
  style.textContent=`
/* 2026-08-18 mobile density + premium card polish */
.category-section{padding:28px 8px 16px!important}.category-section .section-title{margin-bottom:12px!important}.category-grid{grid-auto-rows:auto!important;gap:5px!important}.category-card,.category-card.large{height:205px!important;min-height:205px!important;border-radius:14px!important;box-shadow:0 5px 16px rgba(21,45,34,.09)!important}.category-card.outing{height:168px!important;min-height:168px!important}.category-card h3{letter-spacing:-.03em!important}.jobs-section{padding-top:72px!important}.store-card{border-radius:18px!important;box-shadow:0 12px 34px rgba(18,42,31,.09)!important;border:1px solid #e3e8e4!important}.job-body{padding:20px!important}.job-company{display:none!important}.job-meta{margin-bottom:7px!important;font-size:11px!important;font-weight:700!important;letter-spacing:.04em!important;color:#6a776f!important}.job-body h3{margin-top:0!important;margin-bottom:9px!important;line-height:1.45!important}.job-salary{display:block!important;font-size:20px!important;line-height:1.4!important;color:#173426!important;margin:14px 0 12px!important}.job-conditions{display:grid!important;grid-template-columns:1fr 1fr!important;gap:6px!important;margin-top:0!important}.job-conditions span{display:block!important;padding:10px 11px!important;background:#f4f7f5!important;border-radius:10px!important;font-size:12px!important;line-height:1.5!important}.job-description{display:-webkit-box!important;-webkit-line-clamp:3!important;-webkit-box-orient:vertical!important;overflow:hidden!important}.shop-links{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:6px!important;margin:14px 0 10px!important}.shop-link{width:auto!important;min-height:42px!important;border-radius:10px!important;padding:9px 8px!important;text-align:center!important}.shop-links .shop-link:last-child:nth-child(odd){grid-column:1/-1!important}.apply-link{margin-top:10px!important;padding:13px 15px!important;border:0!important;border-radius:11px!important;background:#173426!important;color:#fff!important;font-size:14px!important;font-weight:800!important;text-decoration:none!important}.job-card[data-job-id='toto-staff'] .job-visual:not(.has-photo){background:radial-gradient(circle at 76% 22%,rgba(214,169,111,.28),transparent 25%),linear-gradient(135deg,#6a4630 0%,#2b1b14 58%,#17110e 100%)!important}.job-card[data-job-id^='share-salon'] .job-visual:not(.has-photo){background:radial-gradient(circle at 82% 16%,rgba(214,225,218,.22),transparent 28%),linear-gradient(135deg,#496a59 0%,#203e30 56%,#10261d 100%)!important}.job-visual:not(.has-photo)::before{content:"LOCAL JOB";position:absolute;top:20px;right:20px;font-size:10px;letter-spacing:.22em;color:rgba(255,255,255,.34);font-weight:700}.job-visual:not(.has-photo) small,.job-visual:not(.has-photo) strong{position:relative;z-index:1;color:#fff!important}.category-intro{margin-bottom:16px!important;padding:24px 26px!important}.category-intro h3{font-size:31px!important;margin-bottom:10px!important}.category-intro-copy>p:last-child{line-height:1.75!important}.category-intro-side ul{margin-bottom:12px!important}.empty-jobs{padding:30px 20px!important}
@media(max-width:700px){.category-section{padding:22px 6px 12px!important}.category-section .section-title{margin-bottom:10px!important}.category-grid{grid-auto-rows:auto!important;gap:4px!important}.category-card,.category-card.large{height:176px!important;min-height:176px!important;padding:12px!important;border-radius:12px!important}.category-card.outing{height:142px!important;min-height:142px!important}.category-card small{font-size:8px!important}.category-card h3{font-size:16px!important;line-height:1.28!important}.category-card>b{right:13px!important;bottom:13px!important;width:34px!important;height:34px!important}.jobs-section{padding-top:56px!important}.jobs-section>.section-title{margin-bottom:16px!important}.category-intro{gap:12px!important;padding:16px!important;margin-bottom:12px!important;border-radius:14px!important}.category-intro h3{font-size:23px!important;margin-bottom:8px!important}.category-intro-copy>p:last-child{font-size:13px!important;line-height:1.65!important}.category-intro-side{padding-top:12px!important}.category-intro-side ul{display:none!important}.category-intro-side strong{font-size:15px!important;margin-bottom:7px!important}.category-intro-side a{font-size:12px!important}.category-selection{margin-bottom:8px!important}.job-visual{min-height:176px!important;padding:17px!important}.job-visual strong{font-size:22px!important}.job-visual:not(.has-photo)::before{top:16px;right:16px;font-size:8px}.job-body{padding:16px!important}.job-body h3{font-size:21px!important}.job-address{font-size:13px!important;margin-bottom:10px!important}.job-description{font-size:13px!important;line-height:1.65!important;margin-bottom:12px!important}.job-salary{font-size:18px!important;margin:11px 0 10px!important}.job-conditions{grid-template-columns:1fr!important}.job-conditions span{padding:9px 10px!important}.shop-links{grid-template-columns:1fr 1fr!important}.shop-link{font-size:11px!important}.apply-link{min-height:46px!important;justify-content:center!important}.empty-jobs{padding:24px 15px!important}}
`;
  document.head.appendChild(style);
})();